...
def deletedot(strlist):
    cop = re.compile(u"[^\u4e00-\u9fa5^a-z^A-Z^0-9]")
    stroutput = []
    for line in strlist:
        line = cop.sub("", line)
        #line=re.sub('[^-^：^.^。^-^(^)^/^~]+','',line)
...
def score_one_quest_one_std(tpllist, stdlist, threshold):
    ...
    for i in range(len(tpllist)):
        score = 0
        for std in stdlist:
            if (fuzz.partial_ratio(tollist[i], std) >= threshold * 100):
...
#     tpl.append(cv.imdecode(np.fromfile(str[i],dtype=np.uint8),-1))
# target =cv.imdecode(np.fromfile(sys.argv[2],dtype=np.uint8),-1)
...
# scores=score_one_quest_one_std(tpl_str_list,std_str_list,0.95)
# for score in scores:
...
im_path = "C:\\Users\\84019\\Desktop\\ocr_all\\wjp-0428\\dcfw\\q2\\SW25-2-1.jpg"
